...
# ---------- OSS config (from your memory: kaelblog bucket, oss-cn-beijing) ----------
OSS_AK = os.environ.get("OSS_AK")
OSS_SK = os.environ.get("OSS_SK")
if not OSS_AK or not OSS_SK:
    raise ValueError("请设置环境变量 OSS_AK 和 OSS_SK")
OSS_BUCKET = "kaelblog"
...
def upload_to_oss(png_path: str, slug: str) -> str:
    """Upload PNG to kaelblog bucket. Returns public URL."""
    try:
        import oss2
    except ImportError:
        sys.exit("oss2 not installed. Run: /Users/nowcoder/miniconda3/bin/pip install oss2")
